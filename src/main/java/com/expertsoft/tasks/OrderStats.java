...
class OrderStats {
    ...
    static List<Order> ordersForCardType(final Stream<Customer> customers, PaymentInfo.CardType cardType) {
        return customers.flatMap(customer -> customer.getOrders()
                .stream().filter(order -> order.getPaymentInfo().getCardType().equals(cardType)))
    ...
    static Map<Integer, List<Order>> orderSizes(final Stream<Order> orders) {
        return orders.collect(Collectors.groupingBy(order -> order.getOrderItems()
    ...
    static Boolean hasColorProduct(final Stream<Order> orders, final Product.Color color) {
        return orders.allMatch(order -> order.getOrderItems()
                .stream().anyMatch(orderItem -> orderItem.getProduct().getColor().equals(color)));
    }
    ...
    static Map<String, Long> cardsCountForCustomer(final Stream<Customer> customers) {
        ...
        return customers.collect(Collectors.toMap(Customer::getEmail, customer -> customer.getOrders()
                .stream()
